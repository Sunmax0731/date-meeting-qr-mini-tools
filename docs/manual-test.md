...
- 作業ディレクトリ: `D:\AI\ChromeExtension\date-meeting-qr-mini-tools`
...
2. `npm run validate` を実行し、`dist/date-meeting-qr-mini-tools-representative-report.json` が生成されることを確認する。
3. レポート JSON を開き、happy-path、missing-required、warning、mixed-batch の summary が期待値通りであることを確認する。
4. Chrome の拡張機能管理画面で extension フォルダを読み込み、popup の主要表示とローカル保存導線を確認する。
